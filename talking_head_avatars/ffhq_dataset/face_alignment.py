...
def image_align_rotate(src_file, dest_file, face_landmarks, output_size=1024, transform_size=4096, enable_padding=True):
        # Align function from FFHQ dataset pre-processing step
        # https://github.com/NVlabs/ffhq-dataset/blob/master/download_ffhq.py
        ...
        if crop_width > crop_height:
            crop = (crop[0] + int((crop_width - crop_height) / 2), crop[1], crop[2] - int((crop_width - crop_height) / 2), crop[3])
        elif crop_height > crop_width:
            crop = (crop[0], crop[1] + int((crop_height - crop_width) / 2), crop[2], crop[3] - int((crop_height - crop_width) / 2))
        ...
        new_crop = (crop[0] + int((bound_w - w) / 2), crop[1] + int((bound_h - h) / 2), crop[2] + int((bound_w - w) / 2), crop[3] + int((bound_h - h) / 2))
