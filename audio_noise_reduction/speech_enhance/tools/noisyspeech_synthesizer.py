...
def main(cfg):
    ...
    if cfg["noisyspeech_dir"] != "None":
        output_dir = cfg["noisyspeech_dir"]
    ...
    noisyspeech_dir = os.path.join(output_dir, "NoisySpeech_training")
    if not os.path.exists(noisyspeech_dir):
        os.makedirs(noisyspeech_dir)
    clean_proc_dir = os.path.join(output_dir, "CleanSpeech_training")
    if not os.path.exists(clean_proc_dir):
        os.makedirs(clean_proc_dir)
    noise_proc_dir = os.path.join(output_dir, "Noise_training")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--config",
        type=str,
        help="Read ./config/config.yaml for all the details",
        default="./config/config.yaml",
    )
    args = parser.parse_args()

    cur_filename = os.path.basename((os.path.relpath(__file__))).split(".")[0]
    # config
    config = yaml.unsafe_load(open(args.config, "r"))
