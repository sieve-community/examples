...
class AudioUpscaler:
    def __init__(self) -> None:
        ...
        self.model = BandwidthExtender.from_pretrained("hifi-gan-bwe-10-42890e3-vctk-48kHz").to(self.device)
    ...
    def upscale(self, audio_path, output_path):
        ...
        with torch.no_grad():
            y = torch.stack([self.model(torch.from_numpy(x).to(self.device), sample_rate) for x in x.T]).T.cpu().numpy()
...
@sieve.Model(
    ...
    system_packages=["libgl1-mesa-glx", "libglib2.0-0", "ffmpeg", "libavcodec58", "libsndfile1"],
    run_commands=[
...
class HiFiGanPlus:
    ...
    def __predict__(self, audio: sieve.Audio) -> sieve.Audio:
        ...
        subprocess.run(["ffmpeg", "-i", audio.path, "-f", "segment", "-segment_time", "20", "-c", "copy", "temp_audio/out%04d.wav"])
        ...
        # concatenate files
        subprocess.run(["ffmpeg", "-f", "concat", "-safe", "0", "-i", "filelist.txt", "-c", "copy", "out.wav"])
