...
def make_animation_batched(source_image, driving_video, inpainting_network, kp_detector, dense_motion_network, avd_network, device, mode = 'standard'):
    ...
    with torch.no_grad():
        ...
        # split driving video into segments of frames, when running on A100 GPU, we can use 1 segment
        driving_video_batched = [driving_video]
        count = 0
        for driving_video_batch in driving_video_batched:
            t = time.time()
            driving = torch.tensor(np.array(driving_video_batch).astype(np.float32)).permute(0, 3, 1, 2).to(device)
            print('time to load video: ', time.time() - t, 's')
            t = time.time()
            if source == None:
                source = torch.tensor(source_image[np.newaxis].astype(np.float32)).permute(0, 3, 1, 2)
                source = source.to(device)
                print('time to load image: ', time.time() - t, 's')
                t = time.time()
                kp_source = kp_detector(source)
                print('time to load kp_source: ', time.time() - t, 's')
                t = time.time()
                print(driving.shape)
                kp_driving_initial = kp_detector(driving[0:1])
                print('time to load kp_driving_initial: ', time.time() - t, 's')
            ...
            for frame_idx in range(driving.shape[0]):
                ...
                datas.append(out['prediction'].data)
                print('time to process frame', count, 'of', len(driving_video), ': ', time.time() - t, 's')
            
            t1 = time.time()
            datas = torch.cat(datas, dim=0)
            all = np.transpose(datas.cpu().numpy(), [0, 2, 3, 1])
            predictions += [all[i] for i in range(all.shape[0])]
            print('time to process batch: ', time.time() - t, 's')

        return predictions


def make_animation(source_image, driving_video, inpainting_network, kp_detector, dense_motion_network, avd_network, device, mode = 'standard'):
    assert mode in ['standard', 'relative', 'avd']
    import time
    with torch.no_grad():
        t = time.time()
        ...
        for frame_idx in range(driving.shape[2]):
            ...
            print('time to process frame: ', time.time() - t, 's')
    return predictions
