...
metadata = sieve.Metadata(
    ...
    description="Create realistic lipsynced avatars from a reference video",
    code_url="https://github.com/sieve-community/examples/blob/main/video-retalking/main.py",
    image=sieve.Image(
        url="https://storage.googleapis.com/sieve-public-data/talking_head_avatar_generation/cover.gif"
    ),
...
class VideoRetalker:
    def __setup__(self):
        ...
        args = Args(face="something.mp4", audio="something.mp3", outfile="./output.mp4")
        (
            self.enhancer,
            self.restorer,
            self.kp_extractor,
    ...
    def __predict__(self, inputs: LipsyncInputs):
        ...
        args = Args(face=face_video.path, audio=audio_file.path, outfile="./output.mp4")
...
@sieve.workflow(name="video_retalking", metadata=metadata)
