...
@sieve.function(
    name="config_generator", gpu=False, python_version="3.8", iterator_input=True
)
...
def mandelbrot_set(x_min, x_max, y_min, y_max, width, height, max_iter):
    ...
    for i in range(width):
        for j in range(height):
            n3[height - j - 1, i] = 256 * np.array(
                colormap[
                    int(
                        mandelbrot_iter(r1[i] + 1j * r2[j], max_iter)
                        / max_iter
                        * (len(colormap) - 1)
                    )
                ]
...
def frame_generator(info: sieve.Struct):
    for i in range(info.n_frames):
        ...
        pad_w = info.initial_zoom / r / 2.0
        pad_h = pad_w * info.height / info.width
        yield sieve.Struct(
            i=i,
            xmin=info.z_real - pad_w,
            xmax=info.z_real + pad_w,
            ymin=info.z_imag - pad_h,
            ymax=info.z_imag + pad_h,
            width=info.width,
            height=info.height,
...
@sieve.function(
    ...
    gpu=False,
    python_packages=["matplotlib==3.6.3"],
    python_version="3.8",
)
def mandelbrot(x: sieve.Struct) -> sieve.Image:
    return sieve.Image(
        ...
        array=mandelbrot_set(
            x.xmin, x.xmax, x.ymin, x.ymax, x.width, x.height, x.max_iter
        ),
    )
...
def mandelbrot_wf(
    ...
    return frame_combine(
        mandelbrot(
            frame_generator(
                config_generator(n_frames, width, height, zoom_ratio, z_real, z_imag)
            )
        )
    )
