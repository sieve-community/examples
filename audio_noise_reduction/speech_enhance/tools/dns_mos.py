...
def main(args):
    ...
    audio_clips_list = glob.glob(
        os.path.join(args.testset_dir, "*.wav")
    )  # glob：搜索列表中符合的文件，返回列表
    ...
    for fpath in audio_clips_list:
        ...
        while try_flag:
            ...
            try:
                ...
                if args.method == "p808":
                    f.write(
                        score_dict["file_name"] + " " + str(score_dict["mos"]) + "\n"
                    )
    ...
    if flag:
        ...
        if args.method == "p808":
            ...
            print(
                "Mean scores for the files: SIG[{}], BAK[{}], OVR[{}]".format(
                    np.mean(df["mos_sig"]),
                    np.mean(df["mos_bak"]),
                    np.mean(df["mos_ovr"]),
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--score_file",
        default=r"./谱修复测试集_yuanjun/noisy/score.csv",
