...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-dist_file", "--dist_file", required=True, type=str, help="output lst"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="the length in time of wav(s)",
    )
    ...
    for dataset_path in candidate_datasets:
        ...
        all_wav_path_list += librosa.util.find_files(
            dataset_path.as_posix(), ext=["wav"]
        )

    all_wav_path_list = offset_and_limit(
        all_wav_path_list, dataset_offset, dataset_limit
    )
