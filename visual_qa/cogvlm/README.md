...
- Simple, concise prompts perform better
- Avoid instructions, especially ones like "Respond with no if xyz is true" as it will tend to always respond with "no" irrespective of what is in the image
- VQA Mode makes its responses a lot more concise. If you would like verbose answers, use the default setup with VQA mode set to False
