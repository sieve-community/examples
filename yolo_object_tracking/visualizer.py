...
def draw_boxes(images: sieve.Image, boxes: List) -> sieve.Image:
    ...
    for i in range(len(boxes_by_frame_number)):
        ...
        for box in boxes:
            ...
            cv2.rectangle(img, (box['box'][0], box['box'][1]), (box['box'][2], box['box'][3]), color, 2)
            cv2.putText(img, box['class_name'], (box['box'][0], box['box'][1]), cv2.FONT_HERSHEY_SIMPLEX, 1, color_lighter, 2)
        
        new_path = f'{uuid.uuid4()}.jpg'
        cv2.imwrite(new_path, img)
        yield sieve.Image(path=new_path, frame_number=boxes[0]['frame_number'], fps=image.fps)
