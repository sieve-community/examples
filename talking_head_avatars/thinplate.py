...
class PlateTalkingHead:
    ...
    def predict(self, source_image, driving_video, fps):
        ...
        for i in range(len(predictions)):
            talking = img_as_ubyte(
                cv2.resize(predictions[i], (crop[2] - crop[0], crop[3] - crop[1]))
            )
            tmp = whole_image.copy()
            tmp[crop[1] : crop[3], crop[0] : crop[2], :] = talking
            tmp = cv2.warpAffine(
                tmp, reverse_angle_matrix, (tmp.shape[1], tmp.shape[0])
            )

            tmp = tmp[
                int(tmp.shape[0] * border_crop) : int(tmp.shape[0] * (1 - border_crop)),
                int(tmp.shape[1] * border_crop) : int(tmp.shape[1] * (1 - border_crop)),
                :,
            ]
        ...
        imageio.mimsave(str(out_path), [frame for frame in predictions], fps=fps)
        return out_path
