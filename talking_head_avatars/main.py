...
class ThinplateAvatar:
    ...
    def detect_faces(self, img):
        ...
        if results.detections:
            for detection in results.detections:
                ...
                # box x1, y1, x2, y2
                box = [
                    int(ratio_box[0] * img_width),
                    int(ratio_box[1] * img_height),
                    int((ratio_box[0] + ratio_box[2]) * img_width),
                    int((ratio_box[1] + ratio_box[3]) * img_height),
                ]
    ...
    def __predict__(self, imgs: sieve.Image, videos: sieve.Video) -> sieve.Video:
        ...
        for img, video in zip(imgs, videos):
            ...
            command = "ffmpeg -i '{}' -i '{}' -c copy -map 0:0 -map 1:1 -shortest '{}'".format(
                out_path, video.path, final_path
            )
...
def thinplate_talking_head(
    driving_video: sieve.Video, driving_audio: sieve.Audio, avatar_image: sieve.Image
) -> sieve.Video:
    ...
    synced = sieve.reference("sieve-developer/wav2lip")(
        driving_video, driving_audio, tracked_faces
    )
